    ...
    pub fn new(size: COORD, input: FileDescriptor, output: FileDescriptor) -> Result<Self, Error> {
        ...
        let result = unsafe {
            (CONPTY.CreatePseudoConsole)(
                ...
                // Netcatty starts each ConPTY with a fresh viewport, so there
                // is no parent cursor position to inherit.  The inherit flag
                // initiates a cursor-report handshake that can leave conhost
                // and ClosePseudoConsole blocked when no host replies.
